...
import org.smartregister.fp.common.adapter.ContactTasksDisplayAdapter;
...
public class ProfileTasksFragment extends BaseProfileFragment implements ProfileFragmentContract.View {
    ...
    private FPFormUtils formUtils = new FPFormUtils();
    ...
    private JSONArray createAccordionValues(JSONObject form) {
        ...
        if (form != null) {
            JSONArray fields = FPFormUtils.fields(form, JsonFormConstants.STEP1);
    ...
    private Task updateTaskValue(JSONArray values) {
        ...
        try {
            if (values != null && values.length() > 0 && newTask != null) {
                ...
                newTask.setComplete(FPJsonFormUtils.checkIfTaskIsComplete(newValue));
            }
